<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <button @click="openModal()" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 text-sm font-medium">+ Add Stock</button>
    </div>
    ...
    <div v-if="store.portfolio" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div class="bg-white rounded-xl shadow-sm p-5">
        ...
        <p class="text-2xl font-bold text-blue-600">{{ formatCurrency(store.portfolio.total_cost) }}</p>
      </div>
      <div class="bg-white rounded-xl shadow-sm p-5">
        ...
        <table v-if="!store.loading" class="w-full text-sm">
          ...
          <tbody>
            <tr v-if="store.items.length === 0">
              <td colspan="8" class="text-center py-10 text-gray-400">No stock holdings found</td>
            </tr>
            <tr v-for="item in store.items" :key="item.id" class="border-b last:border-0 hover:bg-gray-50">
              ...
              <td class="px-4 py-3 text-gray-700">{{ item.company_name }}</td>
              ...
              <td class="px-4 py-3 text-right font-semibold text-blue-600">{{ formatCurrency(stockValue(item)) }}</td>
              <td class="px-4 py-3 text-right font-semibold" :class="stockPL(item) >= 0 ? 'text-green-600' : 'text-red-600'">
                {{ stockPL(item) >= 0 ? '+' : '' }}{{ formatCurrency(stockPL(item)) }}
              </td>
              <td class="px-4 py-3">
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between p-5 border-b">
          <h2 class="font-semibold text-gray-800">{{ editing ? 'Edit Stock' : 'Add Stock' }}</h2>
        ...
        <form @submit.prevent="handleSubmit" class="p-5 space-y-4">
          <div>
            ...
            <input v-model="form.symbol" required class="w-full border rounded-lg px-3 py-2 text-sm uppercase focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. JFC, SM, BDO" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Company Name</label>
            <input v-model="form.company_name" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Number of Shares *</label>
            <input v-model="form.shares" type="number" min="0" step="0.0001" required class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Buy Price (per share) *</label>
            <input v-model="form.buy_price" type="number" min="0" step="0.0001" required class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Current Price (per share)</label>
            <input v-model="form.current_price" type="number" min="0" step="0.0001" class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
    ...
    <div v-if="deleteTarget" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl p-6 max-w-sm w-full shadow-xl">
        ...
        <p class="text-sm text-gray-500 mb-4">Delete {{ deleteTarget.symbol }} holding? This cannot be undone.</p>
...
function stockValue(item) {
  return Number(item.shares || 0) * Number(item.current_price || 0);
}
